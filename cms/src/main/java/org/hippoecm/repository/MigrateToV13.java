...
public class MigrateToV13 {
    ...
    private void doMigrateIfNeeded() throws RepositoryException {
        if (ntm.hasNodeType(NT_MAJOR_RELEASE_MARKER) && !isMigrationStillNeeded()) {
            log.info("No migration needed (includes extra check)");
            return;
        }
        log.info("Start MigrateToV13 (forked)");
    ...
    private boolean isMigrationStillNeeded() throws RepositoryException {
        final String force = System.getProperty("repo.MigrateToV13.force", null);
        final boolean forced = force != null;
        log.warn("{}Doing forced re-run. System property repo.MigrateToV13.force = {}", (forced ? "" : "NOT "), force);

        return forced
            || propertiesExist("hst:configuration",
            new String[]{"hst:lockedby", "hst:lockedon"})
            || propertiesExist("hst:site",
            new String[]{"hst:portalconfigurationenabled"})
            || propertiesExist("hst:virtualhosts",
            new String[]{"hst:channelmanagerhostgroup",
                "hst:prefixexclusions",
                "hst:suffixexclusions"});
    }

    private boolean propertiesExist(final String nodeTypeName, final String[] propertyNames) throws RepositoryException {
        if (!ntm.hasNodeType(nodeTypeName)) {
            log.warn("No node type {}, no migration needed", nodeTypeName);
            return false;
        }

        final NodeType nodeType = ntm.getNodeType(nodeTypeName);
        for (final String propertyName : propertyNames) {
            final PropertyDefinition[] defs = nodeType.getPropertyDefinitions();
            for (int i = 0; i < defs.length; i++) {
                PropertyDefinition propDef = defs[i];
                if (propertyName.equals(propDef.getName())) {
                    log.warn("Found property {}/{}, re-running MigrateToV13", nodeTypeName, propertyName);
                    return true;
                }
            }
            log.warn("Property {}/{} not found, no migration needed", nodeTypeName, propertyNames);
        }
    ...
    private void ensureNodeTypeNotInUse(final String nodeType, boolean mixin, final String[] mixinPrimaryTypes,
        ...
        for (final Node node : nodes) {
            if (mixinPrimaryTypes != null && mixin && !Arrays.stream(node.getMixinNodeTypes()).map(nt -> nt.getName()).anyMatch(nodeType::equals)
                && Arrays.stream(mixinPrimaryTypes).anyMatch(node.getPrimaryNodeType().getName()::equals)) {
