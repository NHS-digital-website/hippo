...
public class WebsiteIconListTransformer extends AbstractSectionTransformer {
    ...
    public ContentNode process() throws ArcException {
        ...
        try {
            ...
            throw new ArcException(String.format("Unable to process IconListItems for %s", this.getClass().getName()), re);
        }

        return sectionNode;
    }

    private void addIconListItems(ContentNode sectionNode) throws ArcException {
        if (listExists(section.getIconlistitems())) {
            for (WebsiteIconListItem item : section.getIconlistitems()) {
                ...
                setSingleProp(iconListItemsNode, WEBSITE_HEADING, item.getHeading());
                ...
                try {
                    ...
                    ContentNode imageNode = setSingleNodeLevelProperty(iconListItemsNode, WEBSITE_IMAGE,
                        HIPPOGALLERYPICKER_IMAGELINK, HIPPO_DOCBASE, imageDocbase.getIdentifier());
                    ...
                    if (listExists(item.getItemlink())) {
                        ...
                        if (isInternal(item.getItemlink())) {
                            ...
                            setSingleNodeLevelProperty(websiteItemLink, WEBSITE_LINK,
                                HIPPO_MIRROR, HIPPO_DOCBASE, linkRefNode.getIdentifier());
                        } else {
                    ...
                    throw new ArcException(String.format("Unable to find the image %s", item.getImageReq()), exception);
                }
            } //* for
        } //* if
    }
