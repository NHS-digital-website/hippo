...
<#-- @ftlvariable name="document" type="uk.nhs.digital.website.beans.Financial" -->
...
<#if (parentWork.publicationStyle)?has_content && (!(document.publicationStyle)?? || document.publicationStyle == 'parent')>
...
<#elseif (document.publicationStyle)?has_content && document.publicationStyle?length gt 0>
    <#if (document.bannerImage)?has_content>
        <@hst.link hippobean=document.bannerImage.original fullyQualified=true var="bannerImage" />
    </#if>
    <#assign publicationStyle = document.publicationStyle!"bluebanner" />
    <#assign bannerImageAltText = document.bannerImageAltText!"" />
</#if>
...
<article class="article article--published-work-chapter"
    ...
    <div class="${gridWrapperClasses?join(" ")}" id="document-content">
        <div class="grid-row">
            <div class="page-block page-block--main">
                <#if hasSummaryContent>
                    <div id="${slugify('Summary')}"
                         class="article-section article-section--summary article-section--reset-top">
                        <h2><@fmt.message key="headers.summary" /></h2>
                        <div data-uipath="website.financial.summary"><@hst.html hippohtml=document.summary contentRewriter=gaContentRewriter/></div>
                    </div>
                </#if>

                <#if document.introductionText?has_content>
                    <div class="article-section article-section--introduction-text">
                        <@hst.html hippohtml=document.introductionText contentRewriter=gaContentRewriter/>
                    </div>
                </#if>

                <#if document.table?has_content>
                    <#list document.table as thisTable>
                        <div class="article-section article-section--table">
                            <@hst.html hippohtml=thisTable contentRewriter=gaContentRewriter/>
                        </div>
                    </#list>
                </#if>
            </div>
