    ...
    <#if section.title?has_content>
        <div id="${slugify(section.title)}" class="${articleSectionClass} <#if isPreviousSectionEmphasisBox>article-section--highlighted</#if>">
            ...
            <#if subHeading>
              ...
              <${subHeadingTag} data-uipath="website.contentblock.section.title"><#if section.isNumberedList>${numberedListCount}. </#if>${section.title}</${subHeadingTag}>
            <#else>
              <#assign mainHeadingTag = "h" + mainHeadingLevel />
              <${mainHeadingTag} data-uipath="website.contentblock.section.title"><#if section.isNumberedList>${numberedListCount}. </#if>${section.title}</${mainHeadingTag}>
            </#if>
