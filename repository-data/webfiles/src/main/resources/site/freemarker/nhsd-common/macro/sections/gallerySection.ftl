...
<#macro gallerySection section mainHeadingLevel=2 prompt=false>
                          ...
                          <#list galleryItem.relatedFiles as attachment>
                            <div class="nhsd-m-card__download-card nhsd-!t-margin-bottom-6">
                              <#if attachment.link.asset?has_content>
                                <@downloadBlockAsset document.class.name attachment.link "${attachment.title}" "" attachment.link.asset.mimeType attachment.link.asset.getLength() false true prompt=prompt/>
                              <#else>
                                <@downloadBlockAsset document.class.name attachment.link "${attachment.title}" "" attachment.link.original.mimeType attachment.link.original.getLength() false true prompt=prompt/>
                              </#if>
                            </div>
                          </#list>
                      </#if>
