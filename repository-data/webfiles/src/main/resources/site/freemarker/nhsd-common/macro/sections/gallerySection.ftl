...
<#macro gallerySection section mainHeadingLevel=2 orgPrompt=false>
    <div id="${slugify(section.heading)}">
        <#if section.headingLevel?has_content && section.headingLevel == 'Main heading'>
            <h2 data-uipath="website.contentblock.gallerysection.title"
                class="nhsd-t-heading-xl">${section.title}</h2>
        <#else>
            <h3 data-uipath="website.contentblock.gallerysection.title"
                class="nhsd-t-heading-l">${section.title}</h3>
        </#if>
                            ...
                            <article class="nhsd-m-card">
                                <div class="nhsd-a-box nhsd-a-box--border-grey">
                                    ...
                                    <div class="nhsd-m-card__image_container">
                                        <figure
                                            class="nhsd-a-image nhsd-a-image--round-top-corners nhsd-a-image--maintain-ratio">
                                    ...
                                    <div class=" nhsd-m-card__content_container">
                                        <div class="nhsd-m-card__content-box">
                                            <#if galleryItem.title?has_content>
                                                <h1 class="nhsd-t-heading-s"
                                                    data-test-text="title">
                                                    ${galleryItem.title}</h1>
                                            </#if>
                                            <#if galleryItem.description?has_content>
                                                <#if galleryItem.description?starts_with("#AT_ONLY") && galleryItem.description?ends_with("AT_ONLY#")>
                                                    <p class="nhsd-t-body-s"
                                                       data-test-text="description">${galleryItem.description}</p>
                                                <#else>
                                        ...
                                        <#if galleryItem.relatedFiles?has_content>
                                            <#list galleryItem.relatedFiles as attachment>
                                                <div
                                                    class="nhsd-m-card__download-card nhsd-!t-margin-bottom-6">
                                                    <#if attachment.title?starts_with("#AT_ONLY") && attachment.title?ends_with("AT_ONLY#")>
                                                        <@downloadBlockAsset "${attachment.class}" "${attachment.link}" "${attachment.title}" "" "${attachment.mimeType}" "${attachment.length}" false true orgPrompt=orgPrompt/>
                                                    <#else>
