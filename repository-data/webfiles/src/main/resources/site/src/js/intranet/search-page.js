...
function loadUrl(url) {
    ...
    if (searchDropdownEvent) searchDropdownEvent.cancel();
...
createSearchDropdown('#globalSearchDropdown', '#query');
function createSearchDropdown(dropdownId, dropdownInputId) {
    const dropdownEl = document.querySelector(dropdownId);
    if (!dropdownEl) return;
    const ajaxSearchUrl = dropdownEl.dataset.liveSearchUrl;
    const searchUrl = dropdownEl.dataset.searchUrl;
    ...
    nhsd(dropdownInputId).on(['input', 'click'], () => {
        searchDropdownEvent = debouncedRequest();
    });
