...
function printPDF() {
    ...
    new Promise((res) => {
        ...
        printPreview.onload = function () {
            setTimeout(() => {
                if (printPreview.contentWindow.document.readyState === 'complete') {
                    res(true);
                } else {
        ...
        printArticle.forEach((url) => {
            ...
            const identity = window.crypto.getRandomValues(new Uint32Array(1))[0].toString(16);
...
const printPDFButtons = Array.from(document.querySelectorAll('.js-print-pdf-button'));
