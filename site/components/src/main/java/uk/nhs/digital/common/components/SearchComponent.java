...
public class SearchComponent extends CommonComponent {
    ...
    private void doSearch(HstRequest request, EssentialsListComponentInfo paramInfo) {
        HippoFacetNavigationBean facetNavigationBean = getFacetNavigationBean(request, paramInfo.getPageSize());
            ...
            if (resultSet != null) {
                Pageable<HippoBean> pageable = getPageableFactory()
                    .createPageable(
                        ...
                        facetNavigationBean.getResultSet().getCount().intValue(),
                        paramInfo.getPageSize(),
                        getCurrentPage(request)
                    );

                request.setAttribute("pageable", pageable);
                request.setAttribute("totalResults", facetNavigationBean.getCount().intValue());
    ...
    HippoFacetNavigationBean getFacetNavigationBean(HstRequest request, int limit) {
    ...
    private HstQuery buildQuery(HstRequest request, Boolean includeWildcards, int limit) {
        ...
        return constructQuery(queryBuilder, searchStringConstraint, getCurrentPage(request), limit);
    }
    ...
    private HstQuery constructQuery(HstQueryBuilder queryBuilder, Constraint searchStringConstraint, int page, int limit) {
        ...
        return queryBuilder.where(and(constraints.toArray(new Constraint[constraints.size() - 1]))).offset(page * limit).limit(limit).build();
    }
