...
public class ApiCatalogueComponent extends ContentRewriterComponent {
    ...
    private static Set<String> userSelectedFilterKeysFrom(final HstRequest request) {
        ...
        // Expectation: Selected filters are submitted through a single 'filters' request query parameter
        // with comma-separated values, e.g: filters=inpatient,deprecated-api

        return Optional.ofNullable(request.getRequestContext())
            .map(HstRequestContext::getBaseURL)
            .map(HstContainerURL::getParameterMap)
            .map(parameterMap -> parameterMap.get(Param.filters.name()))
            .filter(commaDelimitedFilterKeys -> commaDelimitedFilterKeys.length > 0)
            .map(commaDelimitedFilterKeys -> commaDelimitedFilterKeys[0])
            .map(commaDelimitedFilterKeys -> commaDelimitedFilterKeys.split(","))
            .map(Arrays::stream)
