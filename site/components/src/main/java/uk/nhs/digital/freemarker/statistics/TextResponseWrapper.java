...
public class TextResponseWrapper implements ClientHttpResponse {
    ...
    public TextResponseWrapper(ClientHttpResponse originalResponse) {
        this.originalResponse = originalResponse;

        try {
            @SuppressWarnings("UnstableApiUsage")
            String responseBody = String.format("{ \"number\" : %s }",
                CharStreams.toString(new InputStreamReader(originalResponse.getBody(), StandardCharsets.UTF_8)));
            jsonContent = responseBody.getBytes();
        } catch (Exception e) {
