...
public class SearchPageComponent extends CommonComponent {
    ...
    public void jsonResponse(HstRequest request, HstResponse response) {
        ...
        if (contentResults != null) {
            List<JSONObject> documentList = contentResults.getItems().stream()
                .limit(3)
                .map(document -> {
    ...
    public void doBeforeRender(HstRequest request, HstResponse response) {
        ...
        try {
            ...
            request.setAttribute(REQUEST_ATTR_TYPE, getAreas(request));
    ...
    private Pageable<HippoBean> searchContent(HstRequest request, boolean noFilters) {
        List<SearchArea> searchAreas = SearchArea.INDIVIDUAL_DOCUMENT_SEARCH_AREAS;
        List<Team> teams = null;
        List<String> years = null;
        List<String> taxonomies = null;

        if (!noFilters) {
            searchAreas = getAreas(request);
    ...
    private List<SearchArea> getAreas(HstRequest request) {
        ...
        if (areasList != null) {
            List<SearchArea> areas = SearchArea.getSearchAreasFromList(areasList);
            if (!areas.isEmpty()) {
                return areas;
            }
        }
