...
public class JobsFeedComponentTest {
    ...
    @Test
    public void doBeforeRenderHandlesResourceExceptionGracefully() {
        when(resourceServiceBroker.findResources(anyString(), anyString())).thenThrow(new ResourceException("Feed down"));
        ...
        Object jobsAttribute = request.getAttribute("jobList");
        assertThat(jobsAttribute, is(notNullValue()));
        assertTrue(((List<JobDetails>) jobsAttribute).isEmpty());
    }
