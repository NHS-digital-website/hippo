...
public class DocumentUtilsTest {
    ...
    public void includesUpdatesWithNullExpiryDate() {
        ...
        final List<Update> result = DocumentUtils.getFilteredAndSortedUpdates(Collections.singletonList(update));
    ...
    public void excludesUpdatesWithPastExpiryDate() {
        ...
        final List<Update> result = DocumentUtils.getFilteredAndSortedUpdates(Arrays.asList(update1, update2));
    ...
    public void ordersBySeverityAndThenByExpiryDate() {
        ...
        final Calendar calendar1 = new GregorianCalendar(nextYear, Calendar.DECEMBER, 10);
        final Calendar calendar2 = new GregorianCalendar(nextYear, Calendar.JANUARY, 10);
        final Calendar calendar3 = new GregorianCalendar(nextYear, Calendar.MARCH, 10);
        final Calendar calendar4 = new GregorianCalendar(nextYear, Calendar.FEBRUARY, 10);
        final Calendar calendar5 = new GregorianCalendar(nextYear, Calendar.JUNE, 10);

        given(update1.getExpirydate()).willReturn(calendar1);
        given(update1.getSeverity()).willReturn(Update.Severity.IMPORTANT.getText());
        given(update2.getExpirydate()).willReturn(calendar2);
        ...
        given(update4.getSeverity()).willReturn(Update.Severity.IMPORTANT.getText());
        given(update5.getExpirydate()).willReturn(calendar5);
        given(update5.getSeverity()).willReturn(Update.Severity.INFORMATION.getText());
        given(update6.getExpirydate()).willReturn(null);
        given(update6.getSeverity()).willReturn(Update.Severity.IMPORTANT.getText());

        final List<Update> sortedUpdates = DocumentUtils.getFilteredAndSortedUpdates(Arrays.asList(update1, update2, update3, update4, update5, update6));
