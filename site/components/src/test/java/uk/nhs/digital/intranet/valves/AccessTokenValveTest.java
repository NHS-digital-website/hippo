...
public class AccessTokenValveTest {
    ...
    public void requestsNewAccessTokenIfAccessTokenIsExpired() throws Exception {
        final AccessToken expiredAccessToken = new AccessToken("token", "refresh", -3600);
        ...
        when(authorizationProvider.refreshAccessToken(expiredAccessToken)).thenReturn(newAccessToken);
    ...
    public void failsGracefullyIfCannotRequestNewAccessToken() throws Exception {
        ...
        verify(servletResponse).addCookie(CookieProvider.EMPTY_COOKIE);
        verify(valveContext).invokeNext();
    }
}
